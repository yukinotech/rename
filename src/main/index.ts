...
function getDefaults() {
  return {
    ...
    defaultOllamaModel: process.env.OLLAMA_MODEL ?? 'llama3.1',
...
ipcMain.handle('agent:run', async (event, args: { input: string; opts?: AgentRuntimeOptions }) => {
  ...
  const send = (chunk: string | Record<string, unknown>) => {
    ...
    const payloadChunk = isObject
      ...
      : (chunk as string);
    const eventName = isObject && typeof (chunk as Record<string, unknown>).event === 'string'
      ? ((chunk as Record<string, unknown>).event as string)
      : undefined;
    const doneFlag = isObject && 'done' in (chunk as Record<string, unknown>)
  ...
  (async () => {
    try {
      ...
      senderWindow.webContents.send('agent:stream', { taskId, chunk: '', done: true, event: 'end' });
    } catch (error) {
      senderWindow.webContents.send('agent:stream', {
        taskId,
